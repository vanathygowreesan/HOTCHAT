		...
		$(function() {
			...
			$('[data-action="wsf-framework-detect"]').click(function() {
				...
				wsf_obj.framework_detect(function(framework) {
					...
					// Show info message
					wsf_obj.message('<?php echo esc_html(sprintf(
...
<?php
				...
				case 'checkbox' :
?>
<input class="wsf-field wsf-switch" value="1"<?php if($value) { ?> checked<?php } ?><?php
...
?><option value="<?php echo esc_attr($option_value); ?>"<?php if(in_array($option_value, $option_selected)) { ?> selected<?php } ?><?php if($option_disabled) { ?> disabled<?php } ?>><?php echo esc_html($option_text); ?></option>
<?php
					}
?>
</select>
...
?><div data-action="wsf-image-reset" data-for="wsf_<?php echo esc_attr($field); ?>"><?php WS_Form_Common::render_icon_16_svg('delete'); ?></div></div>
<?php
					}
