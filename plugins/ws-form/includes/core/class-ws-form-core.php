	...
	class WS_Form_Core {
			...
			foreach($fields as $field) {
				...
				switch($field) {
					...
					case 'user_id' :
						...
						$value = WS_Form_Common::get_user_id(false);
						$format_set = '%d';
						break;
		...
		public function db_update_insert($table_name, $fields_update, $fields_insert, $object, $object_type, $id = 0, $insert = true) {
			...
			if(is_null($wpdb->get_var("SELECT id FROM $table_name WHERE id = $id LIMIT 1"))) {
				...
				// Truncate label
				if(
					isset($wpdb_update_array['data']['label']) &&
					(strlen($wpdb_update_array['data']['label']) > WS_FORM_FIELD_LABEL_MAX_LENGTH)
				) {

					$wpdb_update_array['data']['label'] = substr($wpdb_update_array['data']['label'], 0, WS_FORM_FIELD_LABEL_MAX_LENGTH);
				}
		...
		public function db_object_sort_index($table_name, $parent_field, $parent_id, $next_sibling_id, $id) {
			...
			$sql = sprintf("SELECT %s FROM %s WHERE id = %u LIMIT 1;", $parent_field, $table_name, $id);
